...
{
    MIFARE_Key key;
    byte aime_key[] = {0x57, 0x43, 0x43, 0x46, 0x76, 0x32};
...
)
{
    ...
    if (no_of_sectors)
    {
        ...
        PICC_DumpMifareClassicSectorToSerial(device, logPrint, uid, key, 0);
    }
...
)
{
    ...
    for (auto i = 0; i < 4; i++)
    {
        for (auto j = 0; j < 18; j++)
        {
            sectorBuffer[i][j] = 0xFF;
        }
    }

    // for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--)
    for (int8_t blockOffset = 0; blockOffset < 4; blockOffset++)
    {
        ...
        if (isSectorTrailer)
        {
            ...
            status = device.PCD_Authenticate(PICC_Command::PICC_CMD_MF_AUTH_KEY_B, firstBlock, key, uid);
            // if (status != StatusCode::STATUS_OK)
            // {
            //     logPrint.print(F("Attempt to Auth with KeyB"));
            //     status = device.PCD_Authenticate(PICC_Command::PICC_CMD_MF_AUTH_KEY_B, firstBlock, key, uid);
            // }
            if (status != StatusCode::STATUS_OK)
            {
        ...
        for (auto i = 0; i < sizeof(buffer); i++)
        {
            ...
            sectorBuffer[blockOffset][i] = buffer[i];
        }
    }

    // logPrint.println("Buffers:");
    for (auto i = 0; i < 4; i++)
    {
        for (auto j = 0; j < 18; j++)
        {
            if (sectorBuffer[i][j] < 0x10)
                // logPrint.print(F("0"));
                ;
            // logPrint.print(sectorBuffer[i][j], HEX);
            // logPrint.print(" ");
        }
        // logPrint.println();
    }
    logPrint.print("Card_No: ");
    for (auto i = 0 + offset; i < 10 + offset; i++)
    {

        if (sectorBuffer[2][i] < 0x10)
            logPrint.print(F("0"));
        logPrint.print(sectorBuffer[2][i], HEX);
    }
    logPrint.println();
